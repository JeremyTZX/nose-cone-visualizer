...
raw_data=readtable("nosecone_inputs_exp2stl.xlsx",'Range','A:E');
isdata= ~isnan(raw_data.D_base);
data=raw_data(isdata,:);
disp(data)

d = data.D_base;   % [in] base diameter of nose cone
...
% convert X,Y,Z coordinates into triangle mesh ----------------------------
    ncmesh=surf2patch(X,Y,Z,'triangles')

% export triangle mesh to output file -------------------------------------
    outputfilename = string(data{i,"file_name"});
    TR=triangulation(ncmesh.faces,ncmesh.vertices);
    stlwrite(TR,outputfilename);
    disp(outputfilename + " successfully saved!")
